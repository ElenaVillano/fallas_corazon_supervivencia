...
Se ajustaron diferentes familias de modelos paramétricos (weibull, lognormal y loglogística) a los datos censurados por derecha resultando el modelo Lognormal con una media de 1.91 y una desviación de 1.91 el que presentó una mejor Log veroscimilitud. Asimismo se realizaron gráficas de diagnóstico, en las que también se observa que el modelo log normal es el que mejor parece adaptarse a los datos.
...
\subsection{Kaplan Mayer}

\begin{figure}[!ht]
\centering
\includegraphics[]{images/niveles.pdf}
\caption{Estimador Kaplan Mayer para las variables dicotómicas.}
\label{fig:km}
\end{figure}
...
Para el presente análisis se utilizó el estimador Kaplan Mayer para determinar la supervivencia de las variables categóricas que tenemos en la muestra. Por ejemplo, en la Figura~\ref{fig:km} obtuvimos el estimador para todas las variables binarias que tenemos, como lo son diabetes, sexo, fumar, anemia y presión alta. Asimismo, se creó una variable que categoriza el nivel de creatinina en alto o bajo. Esta discretización se realizó basand

Cada subgráfico tiene en el título la categoría a la que pertenece. En dicha figura no observamos una supervivencia distinta para grupos que padecen alguna enfermedad (como la diabetes o la anemia) que tradicionalmente suelen estar asociadas a los ataques al corazón. Lo mismo sucede con los grupos de fumadores, en comparación con los no fumadores. 

Asimismo, discretizamos algunas variables continuas, generando intervalos que nos permitieran tener grupos, de acuerdo a umbrales mencionados por la investigación pertinente como relevantes. Siguiendo los resultados de Wannamethee et. Al (1997), generamos grupos a partir de una concentración de 116 micromol/L (micromoles por litro de sangre) de la variable nivel de creatinina, umbral a partir del cual estos autores han detectado, en un estudio basado en otros datos, un riesgo de ataque al corazón. De igual manera, la variable ejection fraction se discretizó en tres grupos, siguiendo a Ahmad et al. (2017). En ambos casos, las tablas Kaplan Meyer muestran un grupo de riesgo con una supervivencia considerablemente menor, lo que sugiere que estas variables podrían tener un efecto explicativo relevante.
